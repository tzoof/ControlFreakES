...
cart_pole_acrobot_best_params = {"centroid": 5.0, "sigma": 5.0, "lambda_mul": 5, "ngen": 400}
mountain_car_best_params = {"centroid": 0.0, "sigma": 50.0, "lambda_mul": 5, "ngen": 2000}
np.random.seed(128)
...
class TrainES:
    ...
    def train(self, maximize_fitness=True):
        ...
        strategy = cma.Strategy(centroid=[self.params["centroid"]] * self.fitness_obj.num_features(), sigma=self.params["sigma"],
                                lambda_=self.params["lambda_mul"] * self.fitness_obj.num_features())
